...
class SOLUTION():
    ...
    def Create_Robot(self):
        ...
        pyrosim.Send_Cube(name="Head", pos=[0, -self.sizes[0][1], maxHeight/2], size=self.sizes[0], color=[1, 0, 0, 1], materialName="Red")# +self.sizes[0][2]/2

        for i in range(1,self.numLinks):
            if i == 1:
                jointHeight = maxHeight/2
                yPosition = -self.sizes[i-1][1]*1.5
            else:
                jointHeight = 0
